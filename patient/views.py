...
def makeGroups(request):
    results = DataPatient.objects.raw('SELECT p.id, count(p.id) as number_patient, p.group_patient as group_patient FROM patient p WHERE p.active = TRUE AND p.classification < 1 GROUP BY p.group_patient, p.id')
